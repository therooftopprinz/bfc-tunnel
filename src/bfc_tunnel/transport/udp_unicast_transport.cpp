...
void udp_unicast_transport::configure(const udp_unicast_transport_config_s& config)
{
    io_reactor->wake_up(
        [config, w = weak_from_this()]()
        {
            ...
            t->is_v6 = config.bind_address.find(':') != std::string::npos;
            if (t->is_v6)
            {
                t->socket = bfc::create_udp6();
            }
            else
            {
            ...
            auto bind_addr = bfc::ip6_port_to_sockaddr(config.bind_address, config.bind_port);

            if (!t->socket.bind(bind_addr))
            {
                log(*g_logger, E_LOG_BIT_ERROR, "udp_unicast_transport[%p]::configure: Failed to bind socket! error=%d(%s)", t.get(), errno, strerror(errno));
                return;
            }
            ...
            auto type = E_TRANSPORT_TYPE_UDP4_UNICAST;
            if (t->is_v6)
            {
                type = E_TRANSPORT_TYPE_UDP6_UNICAST;
            }

            t->out_queue.push(transport_config_s{
                type,
                config.bind_address,
                config.bind_port,
                config.peers.empty() ? std::vector<std::pair<std::string, uint16_t>>() : std::vector<std::pair<std::string, uint16_t>>(config.peers.size(), std::make_pair(config.peers[0].first, config.peers[0].second))
            });
...
void udp_unicast_transport::uninitialize()
{
    ...
    cv_reactor->wake_up(
            [&cv_reactor_promise, w = weak_from_this()]()
            {
                ...
                if (!t)
                {
                    log(*g_logger, E_LOG_BIT_SHOULD_NOT_HAPPEN, "udp_unicast_transport[nullptr]::uninitialize: cv_reactor callback: Weak pointer expired!");
...
{
    socket.send(data.data, 0, (sockaddr*)&data.address, sizeof(data.address));
}
...
void udp_unicast_transport::on_recv_ready()
{
    sockaddr_storage addr;
    // todo: use buffer pool
    constexpr size_t MAX_PDU_SIZE = 1024 * 64;
    bfc::buffer pdu(
        new std::byte[MAX_PDU_SIZE],
        MAX_PDU_SIZE * sizeof(std::byte),
        [](const void* p) { delete[] (std::byte*)p; }
    );
    ...
    if (n > 0)
    {
        if (is_v6)
        {
            out_queue.push(transport6_data_s{*(sockaddr_in6*)&addr, std::move(pdu)});
        }
        else
        {
            out_queue.push(transport4_data_s{*(sockaddr_in*)&addr, std::move(pdu)});
        }
    }
}
